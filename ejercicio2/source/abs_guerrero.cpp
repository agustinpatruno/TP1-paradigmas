...
guerrero::guerrero(tipos_guerreros nombre_guerrero, float fuerza, hab_totales hab_especial, double vida, int armas)
{
    try
    {
        if (corroborar_intervalo(0,fuerza,1000) && corroborar_intervalo(0,static_cast<float>(armas), 2))
        {
            ...
            if (pertenece_hab_combate(hab_especial)) // en caso de que la habilidad sea de las de combate, se usa el 100% del daño
            {
...
unique_ptr<arma> guerrero::crear_arma(armas_totales tipo_arma, float dato1, float dato2, float dato3, double daño)
{
    switch (tipo_arma)
    {
        case arma_baston:
